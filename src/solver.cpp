...
void solver::solve_poisson_equation()
{
    ...
    double Qe      = 1.6E-19;            // 单位电荷量
    double epsilon = 8.84E-14 * 1.00056; // 介电常数
...
void solver::init_uex()
{
    ...
    uex = (Kte < 0.04868).select(1.839E22 / FLOW_ANALYSIS_INIT_MIDPARTICLE_DENSITY, uex);
    uex = (Kte >= 0.04868 && Kte < 0.6217)
              .select((1.27948E22 + 1.06616E23 * Kte - 8.706E22 * (Kte ^ 2)) /
                          FLOW_ANALYSIS_INIT_MIDPARTICLE_DENSITY,
                      uex);
    uex = (Kte >= 0.6217 && Kte < 0.7368)
              .select(4.54284E22 / FLOW_ANALYSIS_INIT_MIDPARTICLE_DENSITY, uex);
    ...
    uex = (Kte >= 85.56 && Kte < 200.0)
              .select((1.64251E18 * Kte + 2.9575E21) / FLOW_ANALYSIS_INIT_MIDPARTICLE_DENSITY, uex);
}
...
void solver::init_Dex()
{
    ...
    Dex = (Kte >= 0.1 && Kte < 1.159)
              .select((exp(52.25793 - 1.12899 / (Kte + 0.20674))) /
                          FLOW_ANALYSIS_INIT_MIDPARTICLE_DENSITY,
                      Dex);
    ...
    Dex = (Kte >= 28.3 && Kte < 31.7)
              .select((5.36739 * 1.0E22 + 8.11836 * 1.0E20 * Kte) /
                          FLOW_ANALYSIS_INIT_MIDPARTICLE_DENSITY,
                      Dex);
...
void solver::init_particle_u_D()
{
    ...
    // 约化电场
    auto EBPx = (f->Ex.cwiseAbs() / FLOW_ANALYSIS_INIT_MIDPARTICLE_DENSITY * 1E17).array();
    auto EBPy = (f->Ey.cwiseAbs() / FLOW_ANALYSIS_INIT_MIDPARTICLE_DENSITY * 1E17).array();

    // x 方向
    // 各粒子的迁移系数
    ion["O-"].ux       = f->pm.molecule["O2-"].ux;
    molecule["O2"].ux  = Matrix::Constant(1.4 * 760 * 300 / (273 * FLOW_ANALYSIS_PRESSURE));
    molecule["O2-"].ux = Matrix::Constant(2.0 * 760 * 300 / (273 * FLOW_ANALYSIS_PRESSURE));

    auto EBPx_tmp = (1.27709 + 4.20304 * 1.0E-4 * EBPx + 9.31504 * 1.0E-6 * (EBPx.pow(2))) * 760 *
                    300 / (273 * FLOW_ANALYSIS_PRESSURE);
    molecule["CO3-"].ux =
        (EBPx > 25).select(1.29 * 760 * 300 / (273 * FLOW_ANALYSIS_PRESSURE), EBPx_tmp);
    molecule["CO2"].ux = Matrix::Constant(1.113 * 760 * 300 / (273 * FLOW_ANALYSIS_PRESSURE));
    ...
    molecule["O2"].uy  = Matrix::Constant(1.4 * 760 * 300 / (273 * FLOW_ANALYSIS_PRESSURE));
    molecule["O2-"].uy = Matrix::Constant(2.0 * 760 * 300 / (273 * FLOW_ANALYSIS_PRESSURE));

    auto EBPy_tmp = (1.27709 + 4.20304 * 1.0E-4 * EBPy + 9.31504 * 1.0E-6 * (EBPy.pow(2))) * 760 *
                    300 / (273 * FLOW_ANALYSIS_PRESSURE);
    molecule["CO3-"].uy =
        (EBPy > 25).select(1.29 * 760 * 300 / (273 * FLOW_ANALYSIS_PRESSURE), EBPy_tmp);
    molecule["CO2"].uy = Matrix::Constant(1.113 * 760 * 300 / (273 * FLOW_ANALYSIS_PRESSURE));
